...
function safeConfig(value) {
    ...
    return {
        ...
        mode: allowedModes.has(String(value.mode)) ? value.mode : 'Free Conversation',
        level: allowedLevels.has(String(value.level)) ? value.level : 'B1',
...
function contentType(path) {
    return { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.svg': 'image/svg+xml', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.ico': 'image/x-icon', '.webp': 'image/webp' }[extname(path)] || 'application/octet-stream';
}
const server = createServer(async (req, res) => {
    try {
        const url = new URL(req.url || '/', `http://${req.headers.host || 'localhost'}`);
        if (req.method === 'GET' && url.pathname === '/health')
            return json(res, 200, { ok: true, service: 'shuvro-tutor' });
        ...
        if (req.method === 'POST' && url.pathname === '/api/token') {
            if (!process.env.LIVEKIT_URL || !process.env.LIVEKIT_API_KEY || !process.env.LIVEKIT_API_SECRET)
                return json(res, 500, { error: 'LiveKit credentials are not configured.' });
            if (!process.env.GOOGLE_API_KEY)
                return json(res, 500, { error: 'GOOGLE_API_KEY is not configured.' });
        ...
        console.error(error);
        if (!res.headersSent)
            json(res, 400, { error: error instanceof Error ? error.message : 'Internal server error' });
    }
});
