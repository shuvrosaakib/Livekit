...
const defaults: TutorSessionConfig = {
  userName: 'Learner',
...
function safeConfig(raw: Partial<TutorSessionConfig>): TutorSessionConfig {
  return {
    ...defaults,
    ...raw,
...
export default defineAgent({
  entry: async (ctx: JobContext) => {
    let config = defaults;
    try {
      const metadata = JSON.parse(ctx.job.metadata || '{}') as Partial<TutorSessionConfig>;
